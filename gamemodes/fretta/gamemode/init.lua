...
function GM:Initialize()
	...
	*/
	...
	if ( GAMEMODE.RoundBased ) then
		timer.Simple( 35, function() GAMEMODE:StartRoundBasedGame() end )
	end
...
function GM:EndOfGame( bGamemodeVote )
	...
	if ( bGamemodeVote ) then
		...
		timer.Simple( GAMEMODE.VotingDelay, function() hook.Run("GAME_MAP_FINISHED") MapVote.Start() end )
